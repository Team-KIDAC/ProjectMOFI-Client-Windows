...
export class ImageDuo implements IImageDuo {
    ...
    init(_data?: any) {
        if (_data) {
            if (Array.isArray(_data["base64String"])) {
                this.base64String = [] as any;
                for (let item of _data["base64String"])
                    this.base64String!.push(item);
            }
        }
    }
    ...
    toJSON(data?: any) {
        data = typeof data === 'object' ? data : {};
        if (Array.isArray(this.base64String)) {
            data["base64String"] = [];
            for (let item of this.base64String)
                data["base64String"].push(item);
        }
        return data;
    }
}
